      ...
      <section>
        ...
        <form action={adAction} className="mt-4 flex flex-wrap items-end gap-3">
          <label className="flex-1 min-w-[200px] text-sm">
            <span className="text-zinc-400">Competitor / brand name</span>
        ...
        {adState.status === "success" && (
          adState.ads.length === 0 ? (
            <p className="mt-4 text-sm text-zinc-500">No active ads found for &ldquo;{adState.searchTerms}&rdquo; in this country.</p>
          ) : (
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {adState.ads.map((ad, i) => (
                <div key={i} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-zinc-100">{ad.pageName}</span>
                    {ad.startedAt && <span className="text-xs text-zinc-600">since {ad.startedAt.slice(0, 10)}</span>}
                  </div>
                  {ad.linkTitles[0] && <p className="mt-2 text-sm font-medium text-amber-300">{ad.linkTitles[0]}</p>}
                  {ad.bodies[0] && <p className="mt-1.5 text-sm text-zinc-400 line-clamp-4">{ad.bodies[0]}</p>}
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {ad.platforms.map((p) => (
                      <span key={p} className="rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] text-zinc-400">{p}</span>
                    ))}
                    {ad.snapshotUrl && (
      ...
      <section>
        ...
        <form action={reportAction} className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="text-sm sm:col-span-2">
            <span className="text-zinc-400">Your product / brand *</span>
          ...
          <label className="text-sm">
            <span className="text-zinc-400">Known competitors (optional)</span>
            <input name="competitors" placeholder="Names you already know" className={`mt-1.5 ${field}`} />
          </label>
          <label className="text-sm">
            <span className="text-zinc-400">Market / geography</span>
        ...
        {reportState.status === "success" && (
          <div className="mt-5 space-y-4">
            <div className="rounded-2xl border border-amber-400/20 bg-amber-950/20 p-5">
              ...
              <p className="mt-1.5 text-sm text-zinc-200">{reportState.result.differentiationOpportunity}</p>
              ...
              <ul className="mt-1.5 space-y-1">
                {reportState.result.recommendedAngles.map((a, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-zinc-300"><span className="mt-0.5 text-amber-400">•</span>{a}</li>
                ))}
              </ul>
            </div>
            ...
            {reportState.result.competitors.map((c, i) => (
              <div key={i} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
                ...
                <p className="mt-1 text-sm text-zinc-400">{c.positioning}</p>
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-500">Strengths</p><p className="mt-1 text-sm text-zinc-300">{c.strengths}</p></div>
                  <div><p className="text-xs font-semibold uppercase tracking-wider text-red-400">Gaps</p><p className="mt-1 text-sm text-zinc-300">{c.gaps}</p></div>
                </div>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {c.estimatedAdAngles.map((a) => (
                    <span key={a} className="rounded-full bg-zinc-800 px-2.5 py-1 text-[11px] text-zinc-400">{a}</span>
                  ))}
                </div>
              </div>
            ))}
