...
**Still gapped** (from the UX-workflow audit, not urgent): campaign strategy version history, credits/billing metering (needs a product decision on the SaaS model first), voice cloning, inpainting/outpainting/style-transfer tools, heatmaps (needs real engagement data). None started — either need a product decision or are lower-value than what's been built.
