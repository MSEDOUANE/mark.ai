...
- **Arabic text rendered directly inside generated ad-creative images** — deferred, and now MORE urgent: as of commit `ff1c12c` the strategist defaults to Arabic, so the default campaign path produces Arabic headlines/CTAs that get overlaid onto creative images by Satori/next-og. Meta ad *body* text handles Arabic fine, but in-image bidi/connected-letterform rendering is still unverified. Worth the spike: render a test creative with `fontFamily` set to an Arabic-supporting font and an Arabic headline, see what actually comes out, before relying on it for real ads.
