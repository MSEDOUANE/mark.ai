...
const FRAMEWORKS = [
  ...
  { id: "Hook-Story-Offer",  label: "Hook-Story-Offer",  desc: "Scroll-stopper → Narrative → CTA" },
  { id: "Benefit-Led",       label: "Benefit-Led",       desc: "Lead with the #1 outcome" },
  { id: "Social Proof",      label: "Social Proof",      desc: "Credibility → Desire → Action" },
  { id: "Fear-of-Missing",   label: "FOMO",              desc: "Urgency, scarcity, exclusivity" },
  { id: "Curiosity Gap",     label: "Curiosity Gap",     desc: "Open a loop the reader must close" },
] as const;

const TONES = ["Conversational", "Professional", "Bold", "Playful", "Luxury", "Urgent"];

const FRAMEWORK_COLORS: Record<string, string> = {
  "AIDA":             "bg-violet-500/20 text-violet-300 border-violet-500/30",
  "PAS":              "bg-blue-500/20 text-blue-300 border-blue-500/30",
  "Hook-Story-Offer": "bg-amber-500/20 text-amber-300 border-amber-500/30",
  "Benefit-Led":      "bg-emerald-500/20 text-emerald-300 border-emerald-500/30",
  "Social Proof":     "bg-pink-500/20 text-pink-300 border-pink-500/30",
  "Fear-of-Missing":  "bg-red-500/20 text-red-300 border-red-500/30",
  "Curiosity Gap":    "bg-teal-500/20 text-teal-300 border-teal-500/30",
};
...
export function AdCopyGenerator({ brands = [] }: { brands?: BrandContextOption[] }) {
  const [state, action, pending] = useActionState<AdCopyState, FormData>(
    generateAdCopy,
    { status: "idle" },
  );
  ...
  async function handleScan() {
    ...
    try {
      ...
      const data = await res.json() as { brandName?: string; description?: string; error?: string };
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_340px]">
      ...
      <form action={action} className="space-y-5">
        {/* Hidden framework checkboxes (always submit selected) */}
        {selectedFrameworks.map((f) => (
          <input key={f} type="hidden" name="frameworks" value={f} />
        ))}
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
          ...
          {scanStatus === "done" && <p className="mt-2 flex items-center gap-1 text-xs text-emerald-400"><svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>Fields filled</p>}
        ...
        {state.status === "success" && !pending && (
          <>
            <div className="flex items-center justify-between">
              <p className="font-semibold">{state.result.variants.length} variants for <span className="text-amber-400">{state.productName}</span></p>
            </div>
