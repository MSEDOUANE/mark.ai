    ...
    <article className="overflow-hidden rounded-2xl border border-app-border bg-app-surface transition-shadow hover:shadow-xl hover:shadow-black/30">
      <div className="p-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-auto text-xs text-app-text-subtle">{entry.window}</span>
        </div>
        <p className="mt-2 text-sm text-app-text-muted">{entry.relevance}</p>
        ...
        <div className="mt-3 rounded-xl bg-amber-950/25 border border-amber-400/20 px-4 py-3">
          ...
          <p className="text-sm text-app-text">{entry.campaignAngle}</p>
        </div>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-app-text-muted">
          <span className="rounded-full bg-app-surface-2 px-2.5 py-1">⏱ {entry.prepLeadTime}</span>
          {entry.suggestedChannels.map((c) => (
            <span key={c} className="rounded-full bg-app-surface-2 px-2.5 py-1 font-medium">{c}</span>
          ))}
        </div>
      </div>
    </article>
      ...
      <div>
        <form action={action} className="space-y-5">
          ...
          <div className="rounded-2xl border border-app-border bg-app-surface/60 p-5 space-y-4">
            ...
            <div>
              <label className="text-sm text-app-text-muted">Planning horizon</label>
      ...
      <div className="space-y-5">
        {state.status === "idle" && !pending && (
          <div className="rounded-2xl border border-dashed border-app-border p-12 text-center">
            ...
            <p className="mt-4 font-medium text-app-text-muted">Your seasonal calendar will appear here</p>
            <p className="mt-1.5 text-sm text-app-text-subtle">Describe the product to get a prioritized calendar of moments to plan around.</p>
          </div>
        )}
        ...
        {pending && (
          <div className="rounded-2xl border border-app-border bg-app-surface/60 p-12 text-center">
            ...
            <p className="mt-3 text-sm text-app-text-muted">Mapping seasonal moments…</p>
          </div>
        )}
          ...
          return (
            <>
              <div className="rounded-2xl border border-app-border bg-app-surface/60 p-5">
                <p className="text-xs font-semibold uppercase tracking-wider text-app-text-subtle mb-2">Calendar for {productName} · {horizon}</p>
                <p className="text-sm text-app-text leading-relaxed">{result.horizonSummary}</p>
                  ...
                  <div className="mt-3 rounded-xl bg-emerald-950/25 border border-emerald-400/20 px-4 py-3">
                    ...
                    <ul className="space-y-1">
                      {result.quickWins.map((q) => (
                        <li key={q} className="flex items-start gap-1.5 text-sm text-app-text"><span className="mt-0.5 text-emerald-400">→</span>{q}</li>
                      ))}
                    </ul>
                  </div>
                )}
