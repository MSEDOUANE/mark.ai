...
- **Dashboard light/dark theme toggle** (commit `8b310ce`) — real, working light theme, not a stub. New `app-*` CSS tokens (globals.css, registered as Tailwind v4 utilities via `@theme`) replace ~2150 chrome-purpose zinc classes across 99 files (scripted, word-boundary-safe migration — `scripts/migrate-theme-tokens.mjs`). Deliberately left Tailwind's built-in zinc/amber palette untouched — `zinc-950` is dual-purpose (page bg AND fixed dark text on ~68 amber CTA buttons), so redefining it would've broken every primary button's contrast. `ThemeToggle` uses zero React state for its own display (pure CSS attribute-driven icon swap) to dodge both a hydration mismatch and the `set-state-in-effect` error class fixed twice already. Persisted via localStorage + anti-FOUC inline script. Visually verified both themes by injecting real dashboard-pattern markup into a live page (nested cards, text hierarchy, the amber CTA button) and toggling `data-theme` — confirmed the button-contrast fix holds in practice. **Known gap**: ~150 residual raw zinc usages (mostly `border-zinc-500` hover borders, low risk) weren't migrated; the actual authenticated dashboard UI itself was never visually verified (no login session available), only the token system and representative markup patterns.

- **Undo for brand/product delete + scheduled-post cancel** (commit — see `git log` for hash) — soft-delete (`deleted_at` on `brand_profiles`/`products`) + a redirect-carried "Undo" banner, same convention as the app's existing info/error banners. Scheduled-post cancel already had a soft "canceled" status; added `restoreScheduledPost`. **Deliberately excludes team-member removal** — soft-deleting a membership would need correctly filtering it out of every permission-check query (`requireManager`, `ensure-profile.ts`, etc.), and a missed spot means a removed member silently keeps access — too high-stakes a mistake for this pass; hard delete + re-invite stays the recovery path. Also **only filters the two main list pages**, not the ~20 other brand/product picker queries scattered across Generate tools (a narrow accepted gap: a just-deleted brand could theoretically still appear in another tool's picker during the undo window).

- **Version history for brand profile edits** — the originally-scoped "campaign strategy version history" didn't hold up on inspection (verified `campaigns.strategy` is write-once, never overwritten after creation by the fatigue-refresh path or anywhere else — there was nothing to version). Brand profile edits are the real analog: colors/tone/voice notes/logo/font/template genuinely do overwrite in place today. New `brand_profile_history` table (one jsonb snapshot per edit, taken pre-overwrite) + a "Version history" panel on the brand edit page with one-click Restore. Restoring itself snapshots the current state first, so it's symmetric/reversible. Live DB round-trip verified against hosted Supabase.

- **Refine-with-feedback across all 11 Generate-hub text tools (2026-07-20)** — user asked whether AI content could "hold the conversation of its creation" and keep improving. Built: `generations` gained `parent_id`/`feedback` columns (a chain = the conversation, no new table); `saveGeneration` now returns the row id; shared helpers `loadRefineParent`/`readRefineFeedback`/`refineDirective` in `tool-context.ts`; a `<RefinePanel>` + `useRefinementRounds` client component (`src/components/refine-panel.tsx`) dropped into all 9 tools (ad-copy, social-captions, personas, audience-insights, marketing-calendar, brand-safety, funnel-design, email-marketing, content-planner). A refine round only ever resubmits `refineGenerationId` + free-text feedback — every tool's `field()` helper now falls back to the parent generation's stored input for everything else. Verified live end-to-end (real Claude call, real DB round-trip) via a throwaway spike script — asked for a specific discount + CTA change, got both applied while the rest of the copy that already worked was kept. Scope cuts made deliberately, not silently: no persistence of the round log across a page refresh (client-side only; the DB chain itself is permanent), and no "restore an older round" (unlike brand-profile history, there's no single row to overwrite here).
- **Video model swap: cheaper custom avatars + higher-quality scene clips (2026-07-19)** — user flagged OmniHuman cost ($0.14/s ≈ $4.20 per 30s BYO-photo avatar video) and low scene-clip quality. Fixed by swapping **models on fal, not providers**: custom avatars now use `fal-ai/kling-video/ai-avatar/v2/standard` ($0.0562/s, **60% cheaper**, 60s audio cap vs OmniHuman's 30s → narration budget doubled to 130 words) via `customAvatarGenerate` in `fal-audio-video.ts`; scene image-to-video moved Kling 2.1 standard → **2.6 Pro** ($0.05→$0.07/s) in `fal-kling-video.ts`. Two 2.6 schema traps handled: param renamed to `start_image_url`, and `generate_audio` **defaults to true which doubles price** — explicitly set `false` (pipeline composes its own TTS). WaveSpeed/Replicate/InfiniteTalk alternatives were priced and rejected (see SESSION_CHANGELOG). **Not live-verified — fal balance still exhausted**; watch the first render after top-up, rollback endpoints documented in both files' comments.

**Still gapped** (from the UX-workflow audit, not urgent): credits/billing metering (needs a product decision on the SaaS model first), voice cloning, inpainting/outpainting/style-transfer tools, heatmaps (needs real engagement data). None started — either need a product decision or are lower-value than what's been built. The full original UX-audit list (command palette, undo, version history, dark mode) is now done.
