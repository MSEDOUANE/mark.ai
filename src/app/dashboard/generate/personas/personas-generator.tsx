    ...
    <article className={`overflow-hidden rounded-2xl border bg-zinc-900 transition-shadow hover:shadow-xl hover:shadow-black/30 ${
      ...
      <div className="flex items-start gap-4 p-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-0.5 text-sm text-zinc-400">{persona.location} · {persona.income}</p>
          {/* Platforms */}
          <div className="mt-2 flex flex-wrap gap-1">
            {persona.preferredPlatforms.map((p) => (
              <span key={p} className="flex items-center gap-1 rounded-full bg-zinc-800 px-2 py-0.5 text-[11px] text-zinc-400">
                {PLATFORM_ICONS[p] ?? ""} {p}
              </span>
            ))}
          </div>
      ...
      {/* Quote */}
      <div className="mx-5 mb-4 rounded-xl bg-zinc-800/50 px-4 py-3">
        <p className="text-sm italic text-zinc-300">&ldquo;{persona.quote}&rdquo;</p>
      </div>
      ...
      <button type="button" onClick={() => setExpanded((p) => !p)}
        className="flex w-full items-center justify-between border-t border-zinc-800 px-5 py-3 text-xs font-medium text-zinc-500 hover:text-zinc-300">
      ...
      {expanded && (
        <div className="grid grid-cols-1 gap-4 border-t border-zinc-800 p-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-600">Core motivation</p>
            <p className="text-sm text-zinc-300">{persona.motivations}</p>
          </div>
...
function Section({ title, items, color }: { title: string; items: string[]; color: string }) {
  return (
    <div>
      <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-600">{title}</p>
      ...
      <div>
        <form action={action} className="space-y-5">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-4">
            ...
            <div>
              <label className="text-sm text-zinc-400">Price point</label>
              <input name="pricePoint" placeholder="800–3 000 MAD" className={`mt-1.5 ${field}`} />
            </div>
            <div>
              <label className="text-sm text-zinc-400">Any existing audience knowledge</label>
      ...
      <div className="space-y-5">
        {state.status === "idle" && !pending && (
          <div className="rounded-2xl border border-dashed border-zinc-800 p-12 text-center">
            ...
            <p className="mt-4 font-medium text-zinc-400">Your buyer personas will appear here</p>
            <p className="mt-1.5 text-sm text-zinc-600">Fill in the product brief to get 3–4 vivid personas with ad hooks.</p>
          </div>
        )}
        ...
        {pending && (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-12 text-center">
            ...
            <p className="mt-3 text-sm text-zinc-400">Building personas from your brief…</p>
          </div>
        )}
          ...
          return (
            <>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Audience summary for {productName}</p>
                <p className="text-sm text-zinc-300 leading-relaxed">{result.audienceSummary}</p>
                <div className="mt-3 rounded-xl bg-amber-950/30 border border-amber-400/20 px-4 py-3">
                  ...
                  <p className="text-sm text-zinc-200">{result.sharedPainPoint}</p>
                </div>
              </div>
