  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="rounded-full bg-zinc-800 px-2.5 py-0.5 text-[11px] font-medium text-zinc-400">{issue.category}</span>
      </div>
      <p className="mt-2.5 border-l-2 border-zinc-600 pl-3 text-sm italic text-zinc-300">“{issue.quote}”</p>
      <p className="mt-2 text-sm text-zinc-400">{issue.explanation}</p>
      <div className="mt-2 rounded-lg bg-emerald-950/25 border border-emerald-400/20 px-3 py-2">
        ...
        <p className="mt-0.5 text-sm text-zinc-200">{issue.suggestedFix}</p>
      </div>
    </div>
  );
}
      ...
      <div>
        <form action={action} className="space-y-5">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-4">
            ...
            <div>
              <label className="text-sm text-zinc-400">Paste your headline, body, or full ad copy *</label>
              <textarea name="copy" rows={7}
                placeholder="Paste the ad copy, caption, or landing-page text you want reviewed…"
                className={`mt-1.5 ${field}`} />
            </div>
            <div>
              <label className="text-sm text-zinc-400">Destination platform</label>
            ...
            <div>
              <label className="text-sm text-zinc-400">Target market</label>
      ...
      <div className="space-y-5">
        {state.status === "idle" && !pending && (
          <div className="rounded-2xl border border-dashed border-zinc-800 p-12 text-center">
            ...
            <p className="mt-4 font-medium text-zinc-400">Your brand-safety report will appear here</p>
            <p className="mt-1.5 text-sm text-zinc-600">Paste copy to get an on-voice + compliance check before you ship.</p>
          </div>
        )}
        ...
        {pending && (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-12 text-center">
            ...
            <p className="mt-3 text-sm text-zinc-400">Reviewing for voice + compliance…</p>
          </div>
        )}
        ...
        {state.status === "success" && !pending && (() => {
          ...
          return (
            <>
              <div className={`rounded-2xl border bg-zinc-900/60 p-5 ${v.ring}`}>
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Verdict</p>
                    <p className={`mt-1 text-2xl font-bold ${v.text}`}>{v.label}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Score</p>
                    <p className="mt-1 text-3xl font-bold text-zinc-100">{result.score}<span className="text-lg text-zinc-500">/100</span></p>
                  </div>
                </div>
                <p className="mt-3 text-sm text-zinc-300 leading-relaxed">{result.summary}</p>
              </div>

              {result.issues.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">{result.issues.length} issue{result.issues.length > 1 ? "s" : ""}</p>
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Strengths</p>
                <ul className="space-y-1">
                  {result.strengths.map((s) => (
                    <li key={s} className="flex items-start gap-1.5 text-sm text-zinc-300"><span className="mt-0.5 text-emerald-400">✓</span>{s}</li>
                  ))}
                </ul>
              </div>

              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">Revised copy</p>
                <p className="whitespace-pre-wrap text-sm text-zinc-200 leading-relaxed">{result.revisedCopy}</p>
              </div>
            </>
          );
        })()}
