    ...
    <div className="space-y-6">
      ...
      {!report && !loading && !error && (
        <div className="rounded-2xl border border-dashed border-zinc-800 p-12 text-center">
          ...
          <p className="mt-4 font-medium text-zinc-400">Paste a website URL to audit its messaging</p>
          <p className="mt-1.5 text-sm text-zinc-600">Get value prop, offers, gaps, and suggested ad angles.</p>
        </div>
      )}
      ...
      {report && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-300">{report.valueProposition}</p>
              </div>
              {campaignHref && (
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-zinc-500">
              <span className="rounded-full bg-zinc-800 px-2.5 py-1">Audience: {report.targetAudience}</span>
              <span className="rounded-full bg-zinc-800 px-2.5 py-1">Tone: {report.toneOfVoice}</span>
            </div>
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Key messages</p>
              <ul className="mt-2 space-y-1.5">
                {report.keyMessages.map((m, i) => <li key={i} className="flex items-start gap-2 text-sm text-zinc-300"><span className="mt-0.5 text-blue-400">•</span>{m}</li>)}
              </ul>
            </div>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Current offers</p>
              {report.currentOffers.length ? (
                <ul className="mt-2 space-y-1.5">
                  {report.currentOffers.map((o, i) => <li key={i} className="flex items-start gap-2 text-sm text-zinc-300"><span className="mt-0.5 text-amber-400">•</span>{o}</li>)}
                </ul>
              ) : <p className="mt-2 text-sm text-zinc-600">No active offers found.</p>}
            </div>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-emerald-500">Strengths</p>
              <ul className="mt-2 space-y-1.5">
                {report.strengths.map((s, i) => <li key={i} className="flex items-start gap-2 text-sm text-zinc-300"><span className="mt-0.5 text-emerald-400">•</span>{s}</li>)}
              </ul>
            </div>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-red-400">Weaknesses</p>
              <ul className="mt-2 space-y-1.5">
                {report.weaknesses.map((w, i) => <li key={i} className="flex items-start gap-2 text-sm text-zinc-300"><span className="mt-0.5 text-red-400">•</span>{w}</li>)}
              </ul>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-amber-400/20 bg-amber-950/20 p-5">
            ...
            <ul className="mt-2 space-y-1.5">
              {report.suggestedAdAngles.map((a, i) => <li key={i} className="flex items-start gap-2 text-sm text-zinc-200"><span className="mt-0.5 text-amber-400">•</span>{a}</li>)}
            </ul>
          </div>
        </div>
      )}
